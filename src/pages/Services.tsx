...
// Add Calendly Modal component
const CalendlyModal = ({ isOpen, onClose, serviceType }: { isOpen: boolean; onClose: () => void; serviceType: string }) => {
  if (!isOpen) return null;

  // Map service types to Calendly URLs
  const calendlyUrls: { [key: string]: string } = {
    sauna: "https://calendly.com/lisasnaturalpath/sauna",
    massage: "https://calendly.com/lisasnaturalpath/massage",
    // Add more service-specific Calendly URLs here
  };

  const calendlyUrl = calendlyUrls[serviceType] || "https://calendly.com/lisasnaturalpath/consultation";
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl w-full max-w-4xl relative">
        ...
        <div className="calendly-inline-widget w-full" data-url={calendlyUrl} style={{ minWidth: '320px', height: '700px' }}></div>
      </div>
    </div>
  );
};
...
export const Services: React.FC = () => {
  ...
  // Add Calendly script when modal is opened
  React.useEffect(() => {
    if (showCalendly) {
  ...
  const handleBooking = (serviceTitle: string, consultationType: string) => {
    if (consultationType === "sauna") {
      setSelectedService("sauna");
      setShowCalendly(true);
    } else if (consultationType === "massage") {
      setSelectedService("massage");
      setShowCalendly(true);
    } else {
      navigate(`/consultation?service=${consultationType}`, {
        state: { serviceTitle, consultationType }
      });
    }
  };
    ...
    <div>
              ...
              >
                ...
                {expandedService === service.title && (
                  <div className="px-6 pb-6">
                    ...
                    <button
                      onClick={() => handleBooking(service.title, service.consultationType)}
      ...
      <section className="py-20 px-4 bg-gray-50">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-4xl font-serif text-brand-purple mb-6">
            Start Your Wellness Journey Today
          </h2>
          <p className="text-xl text-gray-700 mb-12 leading-relaxed">
            Our experienced practitioners are here to guide you on your path to optimal health and well-being. 
            Book your consultation today and take the first step towards a healthier, more balanced life.
          </p>
          <Link
            to="/consultation"
            className="bg-brand-purple hover:bg-brand-purple/90 text-white px-8 py-4 rounded-full inline-flex items-center justify-center text-lg"
          >
